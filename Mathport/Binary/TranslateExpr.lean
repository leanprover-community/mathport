...
def trExpr (e : Expr) : BinportM Expr := do
  ...
  let e ← liftMetaM <| Meta.transform e (pre := translateAutoParams ctx s)
  e
...
where
  ...
  translateAutoParams ctx s e : MetaM TransformStep :=
    ...
    if e.isAppOfArity `Mathlib.auto_param 2 then do
      ...
      try
        ...
        let tacName := mkCandidateLean4NameForKind ctx.config s.nameInfoMap tacName3 ExprKind.eDef
