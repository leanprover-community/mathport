  ...
  if !(vis matches .private) && (← read).config.redundantAlign then
    ...
    if translateToAdditive then
      if let some add4 := ToAdditive.findTranslation? (← getEnv) n4 then
        if let some (add3, _) :=
            (Mathlib.Prelude.Rename.renameExtension.getState (← getEnv)).get.toLean3.find? add4 then
          pushAlign add3 add4
...
def trDecl (dk : DeclKind) (mods : Modifiers) (attrs : Attributes)
  ...
  if s.irreducible then
    ...
    return ← `($mods:declModifiers
      irreducible_def $id.get! $(← trOptDeclSig bis ty):optDeclSig $val:declVal)
  match dk with
  | DeclKind.abbrev => do
