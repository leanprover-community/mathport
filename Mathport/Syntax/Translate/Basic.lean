...
partial def printFirstLineComments : M Unit := do
  let firstComment :: otherComments := (← get).remainingComments | return
  if firstComment.start.line > 1 then return
  printOutput (mkCommentString firstComment)
  modify ({ · with remainingComments := otherComments })
