...
@[tr_tactic propagate_tags] def trPropagateTags : TacM Syntax := do
  `(tactic| propagate_tags $(← trBlock (← itactic)):tacticSeq)

@[tr_tactic intro] def trIntro : TacM Syntax := do
  match ← parse (ident_)? with
  | some (BinderName.ident h) => `(tactic| intro $(mkIdent h):ident)
  | _ => `(tactic| intro)

@[tr_tactic intros] def trIntros : TacM Syntax := do
  match ← parse ident_* with
  | #[] => `(tactic| intros)
  | hs => `(tactic| intro $[$(hs.map trIdent_)]*)

@[tr_tactic introv] def trIntrov : TacM Syntax := do
  `(tactic| introv $((← parse ident_*).map trBinderIdent)*)

@[tr_tactic rename] def trRename : TacM Syntax := do
...
@[tr_tactic apply] def trApply : TacM Syntax := do `(tactic| apply $(← trExpr (← parse pExpr)))

@[tr_tactic fapply] def trFApply : TacM Syntax := do `(tactic| fapply $(← trExpr (← parse pExpr)))

@[tr_tactic eapply] def trEApply : TacM Syntax := do `(tactic| eapply $(← trExpr (← parse pExpr)))
...
@[tr_tactic mapply] def trMApply : TacM Syntax := do `(tactic| mapply $(← trExpr (← parse pExpr)))

@[tr_tactic apply_instance] def trApplyInstance : TacM Syntax := `(tactic| infer_instance)
@[tr_ni_tactic tactic.apply_instance] def trNIApplyInstance (_ : AST3.Expr) : M Syntax :=
  `(tactic| infer_instance)

@[tr_tactic refine] def trRefine : TacM Syntax := do `(tactic| refine' $(← trExpr (← parse pExpr)))

@[tr_tactic assumption] def trAssumption : TacM Syntax := do `(tactic| assumption)

@[tr_tactic assumption'] def trAssumption' : TacM Syntax := do `(tactic| assumption')

@[tr_tactic change] def trChange : TacM Syntax := do
...
@[tr_tactic exact «from»] def trExact : TacM Syntax := do
  `(tactic| exact $(← trExpr (← parse pExpr)))

@[tr_tactic exacts] def trExacts : TacM Syntax := do
...
@[tr_tactic rewrite rw] def trRw : TacM Syntax := do
  let (q, loc) ← trRwArgs; `(tactic| rw [$q,*] $(loc)?)

@[tr_tactic rwa] def trRwA : TacM Syntax := do
  let (q, loc) ← trRwArgs; `(tactic| rwa [$q,*] $(loc)?)

@[tr_tactic erewrite erw] def trERw : TacM Syntax := do
  let (q, loc) ← trRwArgs; `(tactic| erw [$q,*] $(loc)?)

@[tr_tactic with_cases] def trWithCases : TacM Syntax := do
  `(tactic| with_cases $(← trBlock (← itactic)):tacticSeq)

@[tr_tactic generalize] def trGeneralize : TacM Syntax := do
  let h ← parse (ident)? <* parse (tk ":")
  let (e, x) ← parse generalizeArg
  `(tactic| generalize $[$(h.map mkIdent) :]? $(← trExpr e) = $(mkIdent x))

@[tr_tactic induction] def trInduction : TacM Syntax := do
...
@[tr_tactic case] def trCase : TacM Syntax := do
  let args ← parse case
...
@[tr_tactic destruct] def trDestruct : TacM Syntax := do
  `(tactic| destruct $(← trExpr (← parse pExpr)))

@[tr_tactic cases] def trCases : TacM Syntax := do
  let (hp, e) ← parse casesArg
  let e ← trExpr e
  let ids ← parse withIdentList
  match ids with
...
@[tr_tactic cases_matching casesm] def trCasesM : TacM Syntax := do
  let _rec ← parse (tk "*")?
...
@[tr_tactic cases_type] def trCasesType : TacM Syntax := do
  let bang ← parse (tk "!")?
...
@[tr_tactic trivial] def trTrivial : TacM Syntax := `(tactic| trivial)

@[tr_tactic admit «sorry»] def trSorry : TacM Syntax := `(tactic| sorry)

@[tr_tactic contradiction] def trContradiction : TacM Syntax := `(tactic| contradiction)

@[tr_tactic iterate] def trIterate : TacM Syntax := do
...
@[tr_tactic «repeat»] def trRepeat : TacM Syntax := do
  `(tactic| repeat' $(← trBlock (← itactic)):tacticSeq)

@[tr_tactic «try»] def trTry : TacM Syntax := do `(tactic| try $(← trBlock (← itactic)):tacticSeq)

@[tr_tactic skip] def trSkip : TacM Syntax := `(tactic| skip)

@[tr_tactic solve1] def trSolve1 : TacM Syntax := do `(tactic| · ($(← trBlock (← itactic)):tacticSeq))

@[tr_tactic abstract] def trAbstract : TacM Syntax := do
  `(tactic| abstract $(← liftM $ (← parse (ident)?).mapM mkIdentF)?
      $(← trBlock (← itactic)):tacticSeq)

@[tr_tactic all_goals] def trAllGoals : TacM Syntax := do
  `(tactic| all_goals $(← trBlock (← itactic)):tacticSeq)

@[tr_tactic any_goals] def trAnyGoals : TacM Syntax := do
  `(tactic| any_goals $(← trBlock (← itactic)):tacticSeq)

@[tr_tactic focus] def trFocus : TacM Syntax := do
  `(tactic| focus $(← trBlock (← itactic)):tacticSeq)

@[tr_tactic assume] def trAssume : TacM Syntax := do
...
@[tr_tactic «have»] def trHave : TacM Syntax := do
...
@[tr_tactic «let»] def trLet : TacM Syntax := do
  let h := (← parse (ident)?).filter (· != `this) |>.map mkIdent
  let ty ← (← parse (tk ":" *> pExpr)?).mapM (trExpr ·)
  match ← parse (tk ":=" *> pExpr)? with
...
@[tr_tactic «suffices»] def trSuffices : TacM Syntax := do
  let h := (← parse (ident)?).map mkIdent
  let ty ← (← parse (tk ":" *> pExpr)?).mapM (trExpr ·)
  `(tactic| suffices $[$h:ident]? $[: $ty:term]?)

@[tr_tactic trace_state] def trTraceState : TacM Syntax := `(tactic| trace_state)

@[tr_tactic trace] def trTrace : TacM Syntax := do `(tactic| trace $(← trExpr (← expr!)):term)

@[tr_tactic existsi] def trExistsI : TacM Syntax := do
  `(tactic| exists $(← liftM $ (← parse pExprListOrTExpr).mapM trExpr),*)

@[tr_tactic constructor] def trConstructor : TacM Syntax := `(tactic| constructor)

@[tr_tactic econstructor] def trEConstructor : TacM Syntax := `(tactic| econstructor)

@[tr_tactic left] def trLeft : TacM Syntax := `(tactic| left)

@[tr_tactic right] def trRight : TacM Syntax := `(tactic| right)

@[tr_tactic split] def trSplit : TacM Syntax := `(tactic| constructor)

@[tr_tactic constructor_matching] def trConstructorM : TacM Syntax := do
  let _rec ← parse (tk "*")?
  let ps ← liftM $ (← parse pExprListOrTExpr).mapM trExpr
  match _rec with
...
@[tr_tactic exfalso] def trExfalso : TacM Syntax := `(tactic| exfalso)

@[tr_tactic injection] def trInjection : TacM Syntax := do
  let e ← trExpr (← parse pExpr)
  let hs := (← parse withIdentList).map trIdent_' |>.asNonempty
  `(tactic| injection $e $[with $hs*]?)

@[tr_tactic injections] def trInjections : TacM Syntax := do
...
@[tr_tactic simp] def trSimp : TacM Syntax := do
  let autoUnfold ← parse (tk "!")?; let trace ← parse (tk "?")?
  trSimpCore autoUnfold.isSome trace.isSome

@[tr_tactic trace_simp_set] def trTraceSimpSet : TacM Syntax := do
...
@[tr_tactic dsimp] def trDSimp : TacM Syntax := trDSimpCore false false expr?

@[tr_tactic reflexivity refl] def trRefl : TacM Syntax := `(tactic| rfl)
@[tr_ni_tactic tactic.interactive.refl] def trNIRefl (_ : AST3.Expr) : M Syntax := `(tactic| rfl)

@[tr_tactic symmetry] def trSymmetry : TacM Syntax := `(tactic| symm)

@[tr_tactic transitivity] def trTransitivity : TacM Syntax := do
  `(tactic| trans $[$(← liftM $ (← parse (pExpr)?).mapM trExpr)]?)

@[tr_tactic ac_reflexivity ac_refl] def trACRefl : TacM Syntax := `(tactic| ac_rfl)

@[tr_tactic cc] def trCC : TacM Syntax := `(tactic| cc)

@[tr_tactic subst] def trSubst : TacM Syntax := do
  `(tactic| subst $(← trExpr (← parse pExpr)))

@[tr_tactic subst_vars] def trSubstVars : TacM Syntax := `(tactic| subst_vars)

@[tr_tactic clear] def trClear : TacM Syntax := do
  match ← parse ident* with
  | #[] => `(tactic| skip)
  | ids => `(tactic| clear $[$(ids.map mkIdent)]*)

@[tr_tactic dunfold] def trDUnfold : TacM Syntax := do
  let cs ← parse ident*
  let loc ← parse location
...
@[tr_tactic delta] def trDelta : TacM Syntax := do
  `(tactic| delta $(← liftM $ (← parse ident*).mapM mkIdentI)* $[$(← trLoc (← parse location))]?)

@[tr_tactic unfold_projs] def trUnfoldProjs : TacM Syntax := do
  let loc ← parse location
  let cfg ← mkConfigStx? $ (← parseSimpConfig (← expr?)).1.bind quoteSimpConfig
  `(tactic| unfold_projs $[$cfg:config]? $[$(← trLoc loc):location]?)

@[tr_tactic unfold] def trUnfold : TacM Syntax := do
  let cs ← parse ident*
  let loc ← parse location
...
@[tr_tactic unfold1] def trUnfold1 : TacM Syntax := do
  let cs ← parse ident*
  let loc ← parse location
  if (← expr?).isSome then warn! "warning: unsupported: unfold config"
  let cs ← liftM $ cs.mapM mkIdentI
...
@[tr_tactic apply_opt_param] def trApplyOptParam : TacM Syntax := `(tactic| infer_opt_param)

@[tr_tactic apply_auto_param] def trApplyAutoParam : TacM Syntax := `(tactic| infer_auto_param)

@[tr_tactic fail_if_success success_if_fail] def trFailIfSuccess : TacM Syntax := do
...
@[tr_tactic match_target] def trMatchTarget : TacM Syntax := do
  let t ← trExpr (← parse pExpr)
  let m ← expr?
  if m.isSome then warn! "warning: unsupported: match_target reducibility"
  `(tactic| match_target $t)

@[tr_tactic by_cases] def trByCases : TacM Syntax := do
  let (n, q) ← parse casesArg
  let q ← trExpr q
  `(tactic| by_cases $[$(n.map mkIdent) :]? $q)

@[tr_tactic funext] def trFunext : TacM Syntax := do
  `(tactic| funext $[$((← parse ident_*).map trIdent_)]*)

@[tr_tactic by_contradiction by_contra] def trByContra : TacM Syntax := do
  `(tactic| by_contra $[$((← parse (ident)?).map mkIdent):ident]?)

@[tr_tactic type_check] def trTypeCheck : TacM Syntax := do
  `(tactic| type_check $(← trExpr (← parse pExpr)))

@[tr_tactic done] def trDone : TacM Syntax := do `(tactic| done)

@[tr_tactic «show»] def trShow : TacM Syntax := do `(tactic| show $(← trExpr (← parse pExpr)))

@[tr_tactic specialize] def trSpecialize : TacM Syntax := do
...
@[tr_tactic congr] def trCongr : TacM Syntax := do `(tactic| congr)

@[tr_tactic rsimp] def trRSimp : TacM Syntax := do `(tactic| rsimp)

@[tr_tactic comp_val] def trCompVal : TacM Syntax := do `(tactic| comp_val)

@[tr_tactic async] def trAsync : TacM Syntax := do
  `(tactic| async $(← trBlock (← itactic)):tacticSeq)

@[tr_tactic conv] def trConvTac : TacM Syntax := do
  `(tactic| conv
    $[at $((← parse (tk "at" *> ident)?).map mkIdent)]?
    $[in $(← liftM $ (← parse (tk "in" *> pExpr)?).mapM trExpr):term]?
    => $(← trConvBlock (← itactic)):convSeq)

@[tr_conv conv] def trConvConv : TacM Syntax := do
  `(conv| conv => $(← trConvBlock (← itactic)):convSeq)

@[tr_conv skip] def trSkipConv : TacM Syntax := `(conv| skip)

@[tr_conv whnf] def trWhnfConv : TacM Syntax := `(conv| whnf)

@[tr_conv dsimp] def trDSimpConv : TacM Syntax := do
  let o := optTk (← parse onlyFlag)
  let hs ← trSimpArgs (← parse simpArgList)
  let (hs, _all) := filterSimpStar hs -- dsimp [*] is always pointless
  let attrs := (← parse (tk "with" *> ident*)?).getD #[]
...
@[tr_conv trace_lhs] def trTraceLHSConv : TacM Syntax := `(conv| trace_state)

@[tr_conv change] def trChangeConv : TacM Syntax := do
  `(conv| change $(← trExpr (← parse pExpr)))

@[tr_conv congr] def trCongrConv : TacM Syntax := `(conv| congr)

@[tr_conv funext] def trFunextConv : TacM Syntax := `(conv| ext)

@[tr_conv to_lhs] def trToLHSConv : TacM Syntax := `(conv| lhs)

@[tr_conv to_rhs] def trToRHSConv : TacM Syntax := `(conv| rhs)

@[tr_conv done] def trDoneConv : TacM Syntax := `(conv| done)

@[tr_conv find] def trFindConv : TacM Syntax := do
  `(conv| (pattern $(← trExpr (← parse pExpr)):term; ($(← trConvBlock (← itactic)):convSeq)))

@[tr_conv «for»] def trForConv : TacM Syntax := do
...
@[tr_conv simp] def trSimpConv : TacM Syntax := do
  let o := optTk (← parse onlyFlag)
  let hs ← trSimpArgs (← parse simpArgList)
  let attrs := (← parse (tk "with" *> ident*)?).getD #[]
  let hs := (hs ++ attrs.map trSimpExt).asNonempty
...
@[tr_conv guard_lhs] def trGuardLHSConv : TacM Syntax := do
  `(conv| guard_target =ₐ $(← trExpr (← parse pExpr)))

@[tr_conv rewrite rw] def trRwConv : TacM Syntax := do
  let q ← liftM $ (← parse rwRules).mapM trRwRule
  if let some cfg ← expr? then
    warn! "warning: unsupported: rw with cfg: {repr cfg}"
  `(conv| rw [$q,*])
...
@[tr_user_nota format_macro] def trFormatMacro : TacM Syntax := do `(f! $(← trInterpolatedStr))
@[tr_user_nota sformat_macro] def trSFormatMacro : TacM Syntax := do `(s! $(← trInterpolatedStr))

@[tr_tactic min_tac] def trMinTac : TacM Syntax := do
  -- wrong, but better than breakage
  `(tactic| exact minTac $(← trExpr (← parse pExpr)) $(← trExpr (← parse pExpr)))

@[tr_ni_tactic control_laws_tac] def trControlLawsTac (_ : AST3.Expr) : M Syntax :=
  `(tactic| (intros; rfl))

@[tr_tactic blast_disjs] def trBlastDisjs : TacM Syntax := `(tactic| cases_type* or)
