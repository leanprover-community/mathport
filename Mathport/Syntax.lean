...
def synport1 (config : Config) (importRename : NameMap Name)
    (path : Path) (imports : Array Name) : CommandElabM Unit := do
  ...
  let ⟨fmt, _⟩ ← AST3toData4 path ast3 importRename config
